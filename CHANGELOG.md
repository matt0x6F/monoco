...
- **`Monoco-Bump:` commit-message trailers — bump intent for CI releases.** `release` now scans commit messages between the last reachable train tag and `HEAD` for `Monoco-Bump: <module>=<major|minor|patch|skip>` trailers and honors them like `--bump` flags. This closes the CI gap where the apply job (`monoco release -y`, no flags) could only ever cut patches: the PR that deserves a minor declares it, and the declaration rides the merge. Explicit `--bump` flags beat trailers per module; across commits the largest kind wins (`skip < patch < minor < major`); a trailer naming an unknown module or kind fails the plan rather than silently under-releasing. `major` requests remain gated by `--allow-major`/`allow_major`. `--no-trailers` disables the scan. This is still not commit-message inference — no Conventional Commits parsing, no prose classification; the trailer is the same explicit declaration as the flag, carried in git. See [docs/release-model.md](docs/release-model.md#the-monoco-bump-trailer-bump-intent-for-ci-releases).
